...
contract BMPre_ICO {
    ...
    function getDataHolders(address holder)
        external
        constant
        returns (uint256)
    {
    ...
    function getDataHoldersRefBonus(address holder)
        external
        constant
        returns (uint256)
    {
    ...
    function stopPreIco_step2() {
        if (stopBlock != 0 && stopBlock < block.number) {
            ...
            if (uint256(hash) > 0) {
                ...
                uint8 limit = uint8(
                    (amount_investments * 100) / 100000000000000000000000
                );
    ...
    function buy(string promo) payable {
        ...
        if (affiliate != address(0x0)) {
            var (
                ...
                partner_bonus,
                referral_bonus
            ) = contractAffiliate.add_referral(msg.sender, promo, msg.value);
