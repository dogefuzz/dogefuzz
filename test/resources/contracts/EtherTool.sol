...
contract EtherTool {
    ...
    uint256 public currentEventId = 1;

    function getEventId() internal returns (uint256 _result) {
    ...
    function batchTransfer2(address[] _tos, uint256[] _amounts)
        public
        payable
        returns (uint256 _doneNum)
    {
    ...
    function getHash1(
        uint8[5] _winWhiteBall,
        uint8 _winRedBall,
        bytes32 _nonce
    ) public pure returns (bytes32 _result) {
        _result = keccak256(_winWhiteBall, _winRedBall, _nonce);
    }

    function getHash2(address _user, bytes32 _nonce)
        public
        pure
