...
func (s *ConsoleReporterTestSuite) TestSendOutput_ShouldReturnNil_WhenSendValidRequestAndReceiveAValidResponse() {
	...
	buffer.WriteString(fmt.Sprintf("Max Coverage: %0.2f%%\n", 100*(float64(report.Coverage)/float64(report.TotalInstructions))))
	buffer.WriteString(fmt.Sprintf("Average Coverage: %0.2f%%\n", 100*report.AverageCoverage/float64(report.TotalInstructions)))
