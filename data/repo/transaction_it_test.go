...
func (s *TransactionRepoIntegrationTestSuite) TestFindTransactionsByFunctionNameAndOrderByTimestamp_WhenExistsMultipleTransactionsWithTheSameFunction_ShouldReturnListOfTransactions() {
	...
	transactions, err := s.repo.FindTransactionsByFunctionNameAndOrderByTimestamp(s.env.DbConnection().GetDB(), function.Name, 10)
	assert.Nil(s.T(), err)
